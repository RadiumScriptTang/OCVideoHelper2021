class WebRtc {
    ...
    init() {
        if (!this.generateMainHtml()) return
        this.generateVoiceContorl()
        this.generateSplitContorl(this.config.styleList)
        this.setShowHide('.split-select', '#splitContorl')
        this.setShowHide('.voice-volume', '#voiceContorl')
        if (this.toolbarList.indexOf('times') > -1) {
            this.setShowHide('#timesContorl ul', '#timesContorl')
            this.addTimesEvent()
        }
        this.isAutoPlay()
        this.creatVideo()
    ...
    generateMainHtml() {
        ...
        html += this.generateSplitHtml(this.ids.length)
        ...
        html += this.generateToolHtml(this.toolbarList)
        html += '</div>'
        main.innerHTML = html
        return true
    }
    ...
    creatVideo() {
        ...
        for (let i = 0; i < this.ids.length; i++) {
            ...
            if (this.config.playType === 'nonLiveStreaming') {
                quote = {
                    src: {
                        ...
                        devId: this.ids[i],
                        accountToken: this.tokens[i],
                        startTime: this.config.time.startTime,
                        endTime: this.config.time.endTime
                    },
                    autoplay:true
                }
            }
            if (this.config.playType === 'flv') {
                quote = {
                    src:{
                        url: this.ids[i],
                        type:'flv'
            ...
            this.allInstance['type' + (i + 1)] = new KMediaUni(this.params);
            this.allInstance['type' + (i + 1)].loadVideo(quote)
            if(this.config.autoplay) {
                this.asyncs.push(new Promise(resolve => {
                    this.allInstance['type' + (i + 1)].loadedData(() => {
    ...
    allLoading ()  {
        ...
        if (this.config.autoplay) {
            Promise.race(this.asyncs).then(()=> {
                ...
                this.addToolsEvent()
                this.addRepeatEvent()
                this.addFullScrrenEvent()
                this.play('play')

                // 添加播放结束事件
                this.allInstance.type1.addEventListener('ended', () => {
                    document.querySelector('.rtc-mask').style.display = 'flex'
                    this.stop()
                })
            })
        }
        ...
        list.forEach(function (item) {
            switch (item) {
                ...
                case 'sharp':
                    ...
                    break
                case 'times':
                    html += '<div id="timesContorl" class="tool-bar-item tool-btn__' + item + '"><span>倍速</span>'
                    html += '<ul><li id="8">8倍</li><li id="4">4倍</li><li id="2">2倍</li><li id="1" class="times-active">正常倍速</li></ul>'
                    html += '</div>'
                    break
                default:
                    html += '<div class="tool-bar-item tool-btn__' + item + '"></div>'
    ...
    addProgressEvent() {
        ...
        toolProgress.addEventListener('click', function (ev) {
            if (ev.target.className === 'tool-progress' || ev.target.className === 'tool-progress-rate') {
                ...
                that.senTime = rate * that.durationSec
                // 清除进度定时器，防止1s后进度条闪跳
    ...
    setKMediaRate(time) {
        for (let i = 0; i < this.ids.length; i++) {
            this.allInstance['type' + (i + 1)].currentTime(time)
        }
    }
    ...
    addDragEvent() {
        ...
        document.onmousedown = function (e) {
            ...
            document.onmousemove = function (event) {
                ...
                // 进度条设置
                if (!isTarget) {
                    that.allInstance.type1.removeEventListener('currentTimeChange', that.addSpeedRate)
                    that.speedTime = null;
                    // 当前进度位置
                    ...
                    // 拖拽位置对应的已播放时间
                    that.senTime = parseInt(realLeft / rateWidth * that.durationSec)
                    // 设置已播时间的显示
                    that.setViewSenTime(that.senTime)
                }
                // 声音设置
    ...
    play(name) {
        ...
        if (this.playStatus === 'pause') {
            play.style.display = 'block'
            pause.style.display = 'none'
            this.allInstance.type1.removeEventListener('currentTimeChange', this.addSpeedRate)
            this.speedTime = null
        }
        for (let i = 0; i < this.ids.length; i++) {
            if (this.playStatus === 'play') {
    ...
    sharp() {
        ...
        for(let i = 0; i < data.length; i++) {
            ...
            // 重置倍速
            document.querySelector('.times-active').className = ''
            document.querySelector('#timesContorl ul').lastChild.className = 'times-active'
        }
    }
    ...
    scrren() {
        ...
        window.onresize = () => {
            ...
            this.isFull = isFullScreen ? true : false
            if (!this.isFull) {
    ...
    setSpeedRate() {
        ...
        this.allInstance.type1.addEventListener('currentTimeChange', this.addSpeedRate)
    }
    ...
    addSpeedRate(time) {
        ...
        webRTC.senTime = time
        webRTC.setViewSenTime(webRTC.senTime)
        if (webRTC.config.playType === 'liveStreaming' || webRTC.config.playType === 'flv') return
        progress.style.width = webRTC.senTime * 100 / webRTC.durationSec + '%'
        if (progressRatio) {
            progressRatio.style.left = webRTC.senTime * 100 / webRTC.durationSec + '%'
        }
        if (progressTimer) {
            webRTC.setClipProgTimer(progressTimer, webRTC.senTime)
        }
    }
    ...
    initClipPlugin() {
        ...
        jumpTo.onclick = (e) => {
            ...
            this.senTime = Math.round(e.target.offsetLeft * this.durationSec / clipProgress.offsetWidth)
    ...
    creatSelectShadow (id, segment) {
        ...
        // 起始位置比例
        var leftRatio = (sTime - vodStartTime) / (this.durationSec * 1000)
        // 剪辑长度比例
        var widthRatio = (eTime - sTime) / (this.durationSec * 1000)
        // 生成剪辑片段阴影
    ...
    creatScaleLine (ele) {
        ...
        var unit = Math.ceil(this.durationSec / 100)
        var count = Math.floor(this.durationSec / unit) + this.durationSec % unit
    ...
    clipSelectTotalTime (left, width) {
        ...
        // 所选区间比例对应的时段（单位：千毫秒）
        var totalTime = this.durationSec * 1000 * ratio
        totalTime = Math.round(totalTime / 1000) * 1000
        // 区间开始位置的比例对应时间点（单位：千毫秒）
        var startTime = new Date(this.config.time.startTime).getTime() + this.durationSec * 1000 * startRatio
    ...
    setClipSelect(sTime, eTime, value, input) {
        ...
        var startRatio = (sTime - vodStartTime) / this.durationSec / 1000
        var endRatio = (eTime - sTime) / this.durationSec / 1000
        var clipSelectText = document.querySelector('.clip-select-text')
